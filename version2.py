...
# for building frequency band pattern
...
# build mid-frequency band using zig-zag indices
ZZ = zigzag_indices(8)
MID_FREQ = ZZ[10:15]   # only using mid-frequencies
...
def embed_bit_in_block(CQ, R, bit, pos, alpha=ALPHA):
    CQ_mod = CQ.copy()
    R_mod = R.copy()
...
def embed_image(image_path, watermark_text, alpha=ALPHA):
    ...
    for block in dct_blocks:
        ...
        # modify multiple coefficients WITHOUT re-quantizing
        for pos in MID_FREQ:
            i, j = pos
        ...
        # reconstruct final DCT block
        modified_block = (CQ_mod + R_mod) * Q
...
def extract_bit_from_block(CQ_orig, R_orig, CQ_mod, R_mod, pos, alpha=ALPHA):
    i, j = pos
...
def extract_image(orig_rgb, mod_rgb, alpha=ALPHA):
    ...
    for block_orig, block_mod in zip(dct_orig, dct_mod):
        ...
        for pos in MID_FREQ:
            bit = extract_bit_from_block(CQ_orig, R_orig, CQ_mod, R_mod, pos, alpha=alpha)
            votes.append(bit)

        # majority vote
        final_bit = int(round(np.mean(votes)))
